...
class _CreateAccountState extends State<CreateAccount> {
  ...
  Future<bool> staffEmailError(fields) async {
    ...
    if(_email == null ) { 
      ...
      if(!_email.contains("@")) {
        setState(() {
          emailError = fields.emailAlreadyExist;
          emailAlreadyExist = true;                 
        });
        return true;
      }
                      ...
                      child: Column(
                        children: <Widget>[
                          ...
                          _parent == true ? TextFormField(
                            ...
                            decoration: InputDecoration(
                              fillColor: grey,
                              labelText: "${fields.parent} ${fields.pin}",
                              icon: Icon(
                                Icons.vpn_key,
                                color: grey,
                                size: 20.0,
                            ...
                            validator: (str) =>
                               str.isEmpty ? "${fields.parent} ${fields.pin} ${fields.required}"   :  null,
    ...
    if (form.validate()) {
      ...
      FirebaseAuth.instance.fetchSignInMethodsForEmail(email: _email).then((exist) async {
        if(exist.isEmpty) {
          ...
          if(_parent == true) {
            ...
            => StaffLogin(email: _email,)));
              ...
              var snackbar = SnackBar(
                content: Text("${fields.pinDoesntMatch}"),
          ...
          final snackBar = SnackBar(content: Text('${fields.emailAlreadyExist}'), duration: Duration(seconds: 5),);
          mainKey.currentState.showSnackBar(snackBar);
        }
      });
