...
import 'package:kerjoy/admin/main_screen.dart';
import 'package:kerjoy/children.dart';
import 'package:kerjoy/createStaff.dart';
import 'package:kerjoy/createaccount.dart';
import 'package:kerjoy/createchild.dart';
import 'package:kerjoy/login.dart';
import 'package:kerjoy/multi_image/main.dart';
import 'package:kerjoy/newdate.dart';
import 'package:kerjoy/staff_login.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Future<bool> getSnapshotData() async {
    ...
    QuerySnapshot querySnapshot =  await Firestore.instance.collection(childCollection).where('dayCare', isEqualTo: dayCare).getDocuments();
     setState(() {
...
class HorizList extends StatefulWidget {
  ...
  HorizList({this.scrollController, this.indexInt, this.snapShotData, this.vertdrag, this.horizdrag, this.daysSubtotal, this.snapLength}); 
...
class _HorizListState extends State<HorizList> {
  ...
  void initState() {
      ...
      selectedChild.add(widget.snapShotData[widget.indexInt]['childId']);
  ...
  void getRowData(index) async {
    ...
    if(index == null) {
      ...
      data = await Firestore.instance.collection("events").document(dayCare.toString()).collection(todayUtcTimestamp.toString()).document(widget.snapShotData[widget.indexInt]['childId']).collection('eventData').getDocuments();
    } else {
      data = await Firestore.instance.collection("events").document(dayCare.toString()).collection(todayUtcTimestamp.toString()).document(widget.snapShotData[widget.indexInt]['childId']).collection('eventData').getDocuments();
    }
     setState(() {
        ...
        if(index + 7 < 25) {
          if(int.parse(rowDatas.documents[i].documentID)  == index + 7 ) {
            ...
            return GestureDetector(
              ...
              onLongPress: () {
                setState(() {
                  Firestore.instance.collection("events").document(dayCare.toString()).collection(todayUtcTimestamp.toString()).document(widget.snapShotData[widget.indexInt]['childId']).collection('eventData').document((index + 7).toString()).delete();
          ...
          if(int.parse(rowDatas.documents[i].documentID)  == index + 7 - 24  ) {
            ...
            return GestureDetector(
              ...
              onLongPress: () {
                setState(() {
                  Firestore.instance.collection("events").document(dayCare.toString()).collection(todayUtcTimestamp.toString()).document(widget.snapShotData[widget.indexInt]['childId']).collection('eventData').document((index + 7 -24).toString()).delete();
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      image: DecorationImage(
                        image: map[index]['childImage'] != '' ? NetworkImage(map[index]['childImage']) : AssetImage('assets/images/default_profile.png'),
                      ),
