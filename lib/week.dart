...
class _WeekState extends State<Week> {
  ...
  var hour =  DateFormat.H().format(DateTime.now().toUtc().add(Duration(hours: 1)));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: type != 4 ? FloatingActionButton(
        ...
        onPressed: () async {
          await Firestore.instance.collection('events').document(dayCare.toString()).collection("${todayUtcTimestamp.toString()}/${widget.childId}/eventData").document("${hour}").setData({
        
            "time": int.parse(hour.toString()),
